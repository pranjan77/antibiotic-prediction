...
parser.add_argument(
    "antismash_results",
    help="file containing the antismash results for the cluster in a genbank file",
)
parser.add_argument(
    "rgi_results", help="file containing the rgi results for the cluster"
)
parser.add_argument(
    "--output",
    help="set directory to write predictions to, default write to current directory",
)
parser.add_argument(
    "--seed", help="random seed to use for training classifiers", type=int
)
parser.add_argument(
    "--no_SSN",
    help="don't use pfam subfamilies in classification, program will run faster with only small impact on accuracy (default: use sub-PFAMs)",
    nargs="?",
    default=False,
    const=True,
)
parser.add_argument(
    "--blastp_path",
    help="path to blastp executable, only neeeded if using SSN, default is blastp",
)
parser.add_argument(
    "--write_features",
    help="set directory to write features to, default do not write features",
)
parser.add_argument(
    "--antismash_version",
    help="version of antismash used to generate antismash input file, supported versions are 4 and 5, defualt 5",
)
parser.add_argument(
    "--rgi_version",
    help="version of rgi used to generate antismash input file, supported versions are 3 and 5, default 5",
)
...
if args.rgi_version == "5":
    ...
    print(
        "please enter a valid rgi version, program currently accepts output from versions 3 and 5"
    )
    exit()
...
if args.antismash_version == "5":
    ...
    print(
        "please enter a valid antismash version, program currently accepts output from versions 4 and 5"
    )
    exit()


# check validity of files and directories given by user
if not tools.checkIfFileExists(
    antismash_infilename, "antismash"
) or not tools.checkIfFileExists(rgi_infilename, "rgi"):
    exit()
...
if not os.access(out_directory, os.W_OK):
    print(
        "You do not have permission to write to the given output directory, please use a different directory"
    )
    exit()
...
try:
    ...
    is_antibacterial = readFeatureFiles.readClassesMatrix(
        data_path + "feature_matrices/is_antibacterial.csv"
    )
    is_antifungal = readFeatureFiles.readClassesMatrix(
        data_path + "feature_matrices/is_antifungal.csv"
    )
    is_cytotoxic = readFeatureFiles.readClassesMatrix(
        data_path + "feature_matrices/is_cytotoxic.csv"
    )
    is_unknown = readFeatureFiles.readClassesMatrix(
        data_path + "feature_matrices/is_unknown.csv"
    )
    targets_gram_pos = readFeatureFiles.readClassesMatrix(
        data_path + "feature_matrices/targets_gram_pos.csv"
    )
    targets_gram_neg = readFeatureFiles.readClassesMatrix(
        data_path + "feature_matrices/targets_gram_neg.csv"
    )
    full_cluster_list = readFeatureFiles.readClusterList(
        data_path + "feature_matrices/cluster_list_CARD.txt"
    )
except:
    print(
...
if write_features:
    test_features_out = open(
        feature_dir
        + "/"
        + antismash_infilename[
            antismash_infilename.rfind("/") : antismash_infilename.rfind(".")
        ]
        + ".csv",
        "w",
    )
...
is_not_unknown_indices_gram = readFeatureFiles.getNotUnknownIndices(
    is_unknown + target_unannotated
)
...
log_params = {"l1_ratio": 0.05, "alpha": 0.01}

tree_bacterial_prob, tree_bacterial_classifier = tools.treePrediction(
    training_features, y_vars, test_features, tree_params, seed
)
log_bacterial_prob, log_bacterial_classifier = tools.logPrediction(
    training_features, y_vars, test_features, log_params, seed
)
svm_bacterial_prob, svm_bacterial_classifier = tools.svmPrediction(
...
y_vars = is_antieuk
y_vars = y_vars[is_not_unknown_indices]
svm_params = {"kernel": "linear", "C": 0.1}
tree_params = {"depth": None, "n": 25}
log_params = {"l1_ratio": 0.001, "alpha": 0.001}

tree_antieuk_prob, tree_antieuk_classifier = tools.treePrediction(
    training_features, y_vars, test_features, tree_params, seed
)
log_antieuk_prob, log_antieuk_classifier = tools.logPrediction(
    training_features, y_vars, test_features, log_params, seed
)
svm_antieuk_prob, svm_antieuk_classifier = tools.svmPrediction(
    training_features, y_vars, test_features, svm_params, seed
)
...
log_params = {"l1_ratio": 0.0001, "alpha": 0.01}

tree_antifungal_prob, tree_antifungal_classifier = tools.treePrediction(
    training_features, y_vars, test_features, tree_params, seed
)
log_antifungal_prob, log_antifungal_classifier = tools.logPrediction(
    training_features, y_vars, test_features, log_params, seed
)
svm_antifungal_prob, svm_antifungal_classifier = tools.svmPrediction(
    training_features, y_vars, test_features, svm_params, seed
)
...
tree_antitumor_prob, tree_antitumor_classifier = tools.treePrediction(
    training_features, y_vars, test_features, tree_params, seed
)
log_antitumor_prob, log_antitumor_classifier = tools.logPrediction(
    training_features, y_vars, test_features, log_params, seed
)
svm_antitumor_prob, svm_antitumor_classifier = tools.svmPrediction(
    training_features, y_vars, test_features, svm_params, seed
)
...
log_params = {"l1_ratio": 0.05, "alpha": 0.001}

tree_antigramneg_prob, tree_antigramneg_classifier = tools.treePrediction(
    training_features, y_vars, test_features, tree_params, seed
)
log_antigramneg_prob, log_antigramneg_classifier = tools.logPrediction(
    training_features, y_vars, test_features, log_params, seed
)
svm_antigramneg_prob, svm_antigramneg_classifier = tools.svmPrediction(
    training_features, y_vars, test_features, svm_params, seed
)
...
tree_antigrampos_prob, tree_antigrampos_classifier = tools.treePrediction(
    training_features, y_vars, test_features, tree_params, seed
)
log_antigrampos_prob, log_antigrampos_classifier = tools.logPrediction(
    training_features, y_vars, test_features, log_params, seed
)
svm_antigrampos_prob, svm_antigrampos_classifier = tools.svmPrediction(
    training_features, y_vars, test_features, svm_params, seed
)
...
print(
    "tree classifier: "
    + str(tree_bacterial_prob[0, 1])
    + " logistic regression classifier: "
    + str(log_bacterial_prob[0, 1])
    + " svm classifier: "
    + str(svm_bacterial_prob[0, 1])
)
print("probabilities of anti-gram positive activity:")
print(
    "tree classifier: "
    + str(tree_antigrampos_prob[0, 1])
    + " logistic regression classifier: "
    + str(log_antigrampos_prob[0, 1])
    + " svm classifier: "
    + str(svm_antigrampos_prob[0, 1])
)
print("probabilities of anti-gram negative activity:")
print(
    "tree classifier: "
    + str(tree_antigramneg_prob[0, 1])
    + " logistic regression classifier: "
    + str(log_antigramneg_prob[0, 1])
    + " svm classifier: "
    + str(svm_antigramneg_prob[0, 1])
)
print("probabilities of antifungal or antitumor or cytotoxic activity:")
print(
    "tree classifier: "
    + str(tree_antieuk_prob[0, 1])
    + " logistic regression classifier: "
    + str(log_antieuk_prob[0, 1])
    + " svm classifier: "
    + str(svm_antieuk_prob[0, 1])
)
print("probabilities of antifungal activity:")
print(
    "tree classifier: "
    + str(tree_antifungal_prob[0, 1])
    + " logistic regression classifier: "
    + str(log_antifungal_prob[0, 1])
    + " svm classifier: "
    + str(svm_antifungal_prob[0, 1])
)
print("probabilities of antitumor or cytotoxic activity:")
print(
    "tree classifier: "
    + str(tree_antitumor_prob[0, 1])
    + " logistic regression classifier: "
    + str(log_antitumor_prob[0, 1])
    + " svm classifier: "
    + str(svm_antitumor_prob[0, 1])
)
...
try:
    ...
    print(
        "couldn't open output file, please provide an output directory that can be written to"
    )
    exit()

tools.writeProbabilitiesToFile(
    outfile,
    "antibacterial",
    tree_bacterial_prob,
    log_bacterial_prob,
...
tools.writeProbabilitiesToFile(
    outfile,
    "anti-gram positive",
    tree_antigrampos_prob,
    log_antigrampos_prob,
    svm_antigrampos_prob,
)
tools.writeProbabilitiesToFile(
    outfile,
    "anti-gram negative",
    tree_antigramneg_prob,
    log_antigramneg_prob,
    svm_antigramneg_prob,
)
tools.writeProbabilitiesToFile(
    outfile,
    "antifugnal or antitumor or cytotoxic",
    tree_antieuk_prob,
    log_antieuk_prob,
    svm_antieuk_prob,
)
tools.writeProbabilitiesToFile(
    outfile,
    "antifungal",
    tree_antifungal_prob,
    log_antifungal_prob,
    svm_antifungal_prob,
)
tools.writeProbabilitiesToFile(
    outfile,
    "antitumor or cytotoxic",
    tree_antitumor_prob,
    log_antitumor_prob,
    svm_antitumor_prob,
)
