...
SSN_map_filenames = [
    "20800_20799_thiolase_N_term_25_full_ssn_coloredssn_mapping_table",
        ...
        for pfam in SSN_pfam_names:
            ...
            for sequence in pfam_sequences:
                ...
                ssn_seq_filename = (
                    data_path
                    + "SSN/gene_sequences/"
                    + pfam_name_dictionary[pfam]
                    + "_domains.fasta"
                )
                ssn_membership = findSSNMembership(
                    c,
                    sequence,
                    ssn_seq_filename,
                    i,
                    included_clusters[pfam_name_dictionary[pfam]],
                    blast_exe,
                    genome_name,
                    data_path,
                )
                ...
                for cluster in ssn_membership:
                    ...
                    if (
                        pfam_name_dictionary[pfam] + "_" + cluster
                        not in SSN_feature_names
                    ):
                        # print("no match")
                        continue
                    # print "match"
    ...
    for cluster in sequences_in_SSN:
        ...
        for seq in sequences_in_SSN[cluster]:
            ...
            if len(my_out.split()) >= 12:
                ...
                score = -1 * math.log10(
                    (2 ** (-1 * score)) * len(sequence[0]) * len(seq)
                )
